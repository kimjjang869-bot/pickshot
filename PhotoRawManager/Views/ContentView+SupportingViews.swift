        ...
        VStack(spacing: 16) {
            ...
            HStack {
                ...
                if !service.groups.isEmpty {
                    Button("현재 셀렉 학습") {
                        let selected = store.photos.filter { $0.rating >= 4 || $0.isSpacePicked }
                        let rejected = store.photos.filter { $0.rating == 0 && !$0.isSpacePicked && !$0.isFolder && !$0.isParentFolder }
                        StyleLearner.shared.learnFromSelection(selected: selected, rejected: rejected)
                        store.showToastMessage("✅ 스타일 학습 완료 (\(selected.count)장 선택 패턴)")
                    }
                    .buttonStyle(.borderedProminent)
                    .tint(.purple)
